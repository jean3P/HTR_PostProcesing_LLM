...
def create_testing_file(base_dir, dataset, partition, result, train_suggestion, llm_name, name_method, model_ocr):
    # Get current date and time in the format YYYY-MM-DD_HH-MM-SS
    current_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

    # Define the directory structure (without date)
    test_dir = os.path.join(base_dir, dataset, model_ocr, llm_name, name_method, partition)
