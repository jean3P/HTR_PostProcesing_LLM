...
class TextProcessingStrategy(ABC):
    ...
    @abstractmethod
    def check_and_correct_text_line(self, text_line,  pipe, tokenizer, train_set_lines, name_dataset, terminators, logger):
        pass
    ...
    def evaluate_corrected_text(self, original_text_line, corrected_text_line, pipe, tokenizer, terminators, logger):
        logger.info(f"Evaluating the corrected text: '{corrected_text_line}' for the original: '{original_text_line}'")
        system_prompt = (
            f"Act as an 18th-century text line evaluator. Your task is to analyze the original text line by an "
