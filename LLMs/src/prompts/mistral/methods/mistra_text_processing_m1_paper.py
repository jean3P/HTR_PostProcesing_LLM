# src/prompts/methods/mistral_text_processing_m1_paper.py
...
class MistralTextProcessingM1P(TextProcessingStrategy):
    def __init__(self):
    ...
    def check_and_correct_text_line(self, text_line, pipe, tokenizer, train_set_line, name_dataset, logger):
        ...
        corrected_text = self.correct_with_suggestions(text_line, suggestions, pipe, tokenizer, name_dataset, logger)
    ...
    def correct_with_suggestions(self, ocr_text, suggestions, pipe, tokenizer, name_dataset, logger):
        suggestion_part = "\n".join(
        ...
        system_prompt = (
            f"[INST] Act as an {get_time(name_dataset)}-century document analyst specializing in OCR correction. "
